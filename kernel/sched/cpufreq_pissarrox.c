...
#define PISSARROX_UP_RATE_LIMIT_US	0
#define PISSARROX_DOWN_RATE_LIMIT_US	20000
#define PISSARROX_IOWAIT_BOOST_SHIFT	2
...
static bool sugov_should_update_freq(struct sugov_policy *sg_policy, u64 time)
{
	...
	if (policy->governor != &pissarrox_gov ||
 ...
 * @sg_policy: pissarrox policy object to compute the new frequency for.
...
static void sugov_irq_work(struct irq_work *irq_work)
{
	...
	/*
	 * For RT and deadline tasks, the pissarrox governor shoots the
	 * frequency to maximum. Special care must be taken to ensure that this
	 * kthread doesn't result in the same behavior.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the sugov_work() function and before that
	 * the pissarrox governor rejects all other frequency scaling requests.
	 *
...
int pissarrox_set_down_rate_limit_us(int cpu, unsigned int rate_limit_us)
{
	struct cpufreq_policy *policy;
	struct sugov_policy *sg_policy;
	struct sugov_tunables *tunables;
...
EXPORT_SYMBOL(pissarrox_set_down_rate_limit_us);

int pissarrox_set_up_rate_limit_us(int cpu, unsigned int rate_limit_us)
{
	struct cpufreq_policy *policy;
	struct sugov_policy *sg_policy;
	struct sugov_tunables *tunables;
...
EXPORT_SYMBOL(pissarrox_set_up_rate_limit_us);
...
static int sugov_init(struct cpufreq_policy *policy)
{
	...
	tunables->up_rate_limit_us = PISSARROX_UP_RATE_LIMIT_US;
	tunables->down_rate_limit_us = PISSARROX_DOWN_RATE_LIMIT_US;
...
static int __init pissarrox_register(void)
{
	return cpufreq_register_governor(&pissarrox_gov);
}
fs_initcall(pissarrox_register);
